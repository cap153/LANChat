...
async fn main() {
    ...
    let pool = lanchat_lib::db::init_db_standalone(db_path)
        .await
        .expect("数据库初始化失败");
    
    // 从数据库读取用户名和 ID
    let my_name = lanchat_lib::db::get_username(&pool)
        .await
        .unwrap_or_else(|_| "Web-User".to_string());
    
    let my_id = lanchat_lib::db::get_user_id(&pool)
        .await
        .expect("无法获取或生成用户 ID");
    ...
    tokio::spawn(async move {
        lanchat_lib::web_server::start_server(port, port, pool_clone, peer_manager_clone).await;
    });
    ...
    let peer_manager_clone = peer_manager.clone();
    tokio::spawn(async move {
        lanchat_lib::network::discovery::start_listening(port, listen_id, listen_name, peer_manager_clone).await;
    });
    ...
    tokio::spawn(async move {
        lanchat_lib::network::discovery::start_announcing(port, announce_id, announce_pool).await;
    });
